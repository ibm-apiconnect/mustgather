...
version_ge() {
  ...
  if which kubectl-cnp >/dev/null && CURRENT_CNP_VERSION=$(kubectl-cnp version  | grep -oE '[0-9]+\.[0-9]+\.[0-9]+') && version_ge "$REQUIRED_CNP_VERSION" "$CURRENT_CNP_VERSION"; then
      ...
      echo -e "kubectl-cnp plugin <= v1.22.4 not found"
      ...
      case $yn in
          [Yy]* )
              ...
              ;;
          [Nn]* )
              echo -e "Exiting... please install kubectl-cnp plugin version v1.22.4 and add it to your PATH, see https://www.enterprisedb.com/docs/postgres_for_kubernetes/latest/kubectl-plugin."
              exit 1
              ;;
      esac
  fi
